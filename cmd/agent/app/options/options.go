...
func (options *VolcanoAgentOptions) Validate() error {
	if options.OverSubscriptionRatio <= 0 || options.OverSubscriptionRatio > 100 {
		return fmt.Errorf("over subscription ratio must be greater than 0 and no more than 100")
	}
	return nil
}
