...
func (c *resetCmd) run() (err error) {
	...
	if err != nil {
		if errors.Is(err, cilliumbpf.ErrKeyNotExist) || os.IsNotExist(err) {
			fmt.Fprintf(c.out, "throttling config does not existed, reset successfully")
			klog.InfoS("Network QoS command called successfully, throttling config does not existed")
			return nil
		}
