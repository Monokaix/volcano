...
var _ = ginkgo.Describe("Job E2E Test", func() {
	...
	ginkgo.It("Schduling gates block vcjob. vcjob allocated after removed.", func() {
		...
		//Remove the scheduling gates at pod level
		patchData := []byte(`[
			{
				"op": "replace",
		...
		for _, pod := range e2eutil.GetTasksOfJob(ctx, queue1Job1) {
			_, err = ctx.Kubeclient.CoreV1().Pods(job.Namespace).Patch(context.TODO(), pod.Name, types.JSONPatchType, patchData, metav1.PatchOptions{})
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
		}
	...
	ginkgo.It("Schduling gates block Deployment. Deployment allocated after removed.", func() {
		...
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		gomega.Expect(len(pgList.Items)).To(gomega.Equal(1))
		pg := &pgList.Items[0]
		...
		for _, pod := range podList.Items {
			...
			if strings.HasPrefix(pod.Name, dep.Name) {
				...
				_, err = ctx.Kubeclient.CoreV1().Pods(dep.Namespace).Patch(context.TODO(), dep.Namespace, types.JSONPatchType, patchData, metav1.PatchOptions{})
				gomega.Expect(err).NotTo(gomega.HaveOccurred())
			}
		}
