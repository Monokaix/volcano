...
type Getter interface {
	...
	UsagesByValue(includeGuaranteedPods, includeSystemUsage bool) Resource
...
// UsagesByValue return absolute resource usage of node
func (g *getter) UsagesByValue(includeGuaranteedPods, includeSystemUsage bool) Resource {
	return g.commonUsage(includeGuaranteedPods, includeSystemUsage)
}

// UsagesByPercentage return resource usage percentage of node
func (g *getter) UsagesByPercentage(node *v1.Node) Resource {
	res := make(Resource)
	usage := g.commonUsage(true, g.collector.Config.GenericConfiguration.IncludeSystemUsage)
...
func (g *getter) commonUsage(includeGuaranteedPods, includeSystemUsage bool) Resource {
	res := make(Resource)
	...
	metricInfos := map[v1.ResourceName]*local.LocalMetricInfo{
		v1.ResourceCPU:    {ResourceType: "cpu", IncludeGuaranteedPods: includeGuaranteedPods, IncludeSystemUsed: includeSystemUsage},
		v1.ResourceMemory: {ResourceType: "memory", IncludeSystemUsed: includeSystemUsage},
	}
