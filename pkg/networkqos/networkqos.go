...
func GetOnlineBandwidthWatermark(serverRateQuota int, qosConf *api.NetworkQos) (string, error) {
	...
	return strconv.Itoa(serverRateQuota*onlineBandwidthWatermarkPercent/100) + "Mbps", nil
}

func GetOfflineLowBandwidthPercent(serverRateQuota int, qosConf *api.NetworkQos) (string, error) {
	...
	return strconv.Itoa(serverRateQuota*offlineLowBandwidthPercent/100) + "Mbps", nil
}

func GetOfflineHighBandwidthPercent(serverRateQuota int, qosConf *api.NetworkQos) (string, error) {
	...
	return strconv.Itoa(serverRateQuota*offlineHighBandwidthPercent/100) + "Mbps", nil
}
...
func InstallNetworkQoS() error {
	...
	defer cancel()
	cmd := fmt.Sprintf("sudo /bin/cp -f /usr/local/bin/bwm_tc.o /usr/share/bwmcli")
	output, err := exec.GetExecutor().CommandContext(ctx, cmd)
	if err != nil {
		klog.ErrorS(err, "Failed to install bwm_tc.o to /usr/share/bwmcli; %s", output)
		return err
	}

	cmd = fmt.Sprintf("sudo /bin/cp -f /usr/local/bin/network-qos /opt/cni/bin")
	output, err = exec.GetExecutor().CommandContext(ctx, cmd)
	if err != nil {
		klog.ErrorS(err, "Failed to install network-qos to /opt/cni/bin; %s", output)
		return err
	}
