...
func (sc *SchedulerCache) DeleteHyperNode(obj interface{}) {
	...
	sc.HyperNodesInfo.MarkHyperNodeIsDeleting(hn.Name)
	if err := sc.HyperNodesInfo.UpdateAncestors(hn.Name); err != nil {
		klog.ErrorS(err, "Failed to update Ancestor in cache", "name", hn.Name)
		return
	}
	sc.HyperNodesInfo.RemoveParent(hn.Name)

	// We can safely delete hyperNode after updated ancestors.
	sc.HyperNodesInfo.DeleteHyperNode(hn.Name)
...
func (sc *SchedulerCache) updateHyperNode(hyperNode *topologyv1alpha1.HyperNode) error {
	sc.HyperNodesInfo.UpdateHyperNode(hyperNode)
	return sc.HyperNodesInfo.UpdateAncestors(hyperNode.Name)
}
