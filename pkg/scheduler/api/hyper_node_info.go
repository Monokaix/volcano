...
func (hni *HyperNodesInfo) BuildHyperNodeCache(hn *HyperNodeInfo, processed sets.Set[string], ancestors sets.Set[string], nodes []*corev1.Node) error {
	...
	hni.setReady(true)
	klog.V(0).InfoS("Successfully built RealNodesSet with members", "name", hn.Name, "nodeSets", hni.realNodesSet[hn.Name])
	return nil
}
...
func (hni *HyperNodesInfo) RemoveFromTierSet(hyperNodeName string, oldTier int) {
	set, ok := hni.hyperNodesListByTier[oldTier]
	if !ok {
		return
	}
	...
	if set.Len() == 0 {
		delete(hni.hyperNodesListByTier, oldTier)
	}
}

// GetAncestors returns all ancestors of a given HyperNode.
func (hni *HyperNodesInfo) GetAncestors(hyperNodeName string) sets.Set[string] {
	ancestors := sets.New[string](hyperNodeName)
	...
	for len(queue) > 0 {
		...
		if parent, ok := hni.parentMap[current]; ok {
...
func (hni *HyperNodesInfo) UpdateAncestors(name string) error {
	...
	for ancestor := range ancestors {
		if hn, ok := hni.hyperNodes[ancestor]; ok {
			if err := hni.BuildHyperNodeCache(hn, processed, ancestorsChain, nodes); err != nil {
				return err
			}
		}
	}
...
func (hni *HyperNodesInfo) UpdateHyperNode(hyperNode *topologyv1alpha1.HyperNode) {
	tier, err := getTier(hyperNode)
	if err != nil {
		klog.ErrorS(err, "Failed to tet tier", "hyperNodeName", hyperNode.Name)
		return
	}
...
// getTier retrieves the tier of a HyperNode.
func getTier(hyperNode *topologyv1alpha1.HyperNode) (int, error) {
	tier, err := strconv.Atoi(hyperNode.Spec.Tier)
	if err != nil {
		return -1, fmt.Errorf("failed to convert tier to int, hyperNodeName: %s, err: %v, ", hyperNode.Name, err)
	}
	return tier, nil
}
...
func (hni *HyperNodesInfo) removeFromTierSet(hyperNodeName string, tier int) {
	set, ok := hni.hyperNodesListByTier[tier]
	if !ok {
		return
	}
	...
	set.Delete(hyperNodeName)
	if set.Len() == 0 {
		delete(hni.hyperNodesListByTier, tier)
	}
}
...
func (hni *HyperNodesInfo) updateHyperNodeListByTier(hyperNode *topologyv1alpha1.HyperNode, tier int) {
	if _, ok := hni.hyperNodesListByTier[tier]; !ok {
		hni.hyperNodesListByTier[tier] = sets.New[string]()
	}
	hni.hyperNodesListByTier[tier].Insert(hyperNode.Name)
}
...
// DeleteHyperNode deletes a HyperNode from the cache.
