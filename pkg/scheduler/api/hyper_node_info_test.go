...
func TestBuildHyperNodeCache_MultipleParents(t *testing.T) {
	selector := "exact"
	s0 := BuildHyperNode("s0", 1, topologyv1alpha1.MemberTypeNode, []string{"node-0", "node-1"}, selector)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			hni.hyperNodes["s2"] = NewHyperNodeInfo(s22, 2)
			err := hni.updateAncestors(s22.Name)
			assert.NoError(t, err)
			assert.Equal(t, tt.expectedRealNodesSet, hni.realNodesSet)
			assert.Equal(t, tt.expectedParentMap, hni.parentMap)
