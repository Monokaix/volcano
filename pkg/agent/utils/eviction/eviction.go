...
func evictPod(ctx context.Context, client clientset.Interface, gracePeriodSeconds *int64, pod *v1.Pod) error {
...
type Eviction interface {
	Evict(ctx context.Context, pod *v1.Pod, eventRecorder record.EventRecorder, gracePeriodSeconds int64, evictMsg string) bool
}
...
func (e *eviction) Evict(ctx context.Context, pod *v1.Pod, eventRecorder record.EventRecorder, gracePeriodSeconds int64, evictMsg string) bool {
	if types.IsCriticalPod(pod) {
		klog.ErrorS(nil, "Cannot evict a critical pod", "pod", klog.KObj(pod))
		return false
	}

	eventRecorder.Eventf(pod, v1.EventTypeWarning, Reason, evictMsg)
