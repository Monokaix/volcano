...
type Interface interface {
	...
	SupportOverSubscription(node *v1.Node) bool
	// ShouldEvict return whether we should evict low priority pods.
	ShouldEvict(node *v1.Node, resName v1.ResourceName, resList *utilnode.ResourceList, hasPressure bool) bool
	...
	ShouldUpdateOverSubscription(node *v1.Node, resource apis.Resource) bool
...
type EvictionCtx struct {
	*config.Configuration
	...
	Filter              func(resName v1.ResourceName, resList *utilnode.ResourceList) bool
}

func EvictPods(ctx *EvictionCtx) error {
	evict := func(node *v1.Node) (bool, error) {
		keepGoing := false
