...
func TestResourcesHandle_Handle(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			prepare: func() {
				prepare(t, "uid1", containerID1, containerID2)
			},
			post: func() map[string]string {
				return file.ReadBatchFromFile([]string{
					"/fake/cpu/kubepods/burstable/poduid1/65a6099d/cpu.shares",
					"/fake/cpu/kubepods/burstable/poduid1/65a6099d/cpu.cfs_quota_us",
					"/fake/cpu/kubepods/burstable/poduid1/13b017b7/cpu.shares",
					"/fake/memory/kubepods/burstable/poduid1/13b017b7/memory.limit_in_bytes",
					"/fake/cpu/kubepods/burstable/poduid1/cpu.shares",
				})
			},
			wantErr: false,
			wantVal: map[string]string{
				// container1
				"/fake/cpu/kubepods/burstable/poduid1/65a6099d/cpu.shares":       "512",
				"/fake/cpu/kubepods/burstable/poduid1/65a6099d/cpu.cfs_quota_us": "200000",

				// container2
				"/fake/cpu/kubepods/burstable/poduid1/13b017b7/cpu.shares":               "1024",
				"/fake/memory/kubepods/burstable/poduid1/13b017b7/memory.limit_in_bytes": "10737418240",

				// pod
				"/fake/cpu/kubepods/burstable/poduid1/cpu.shares": "1536",
			},
		},
	}
...
func prepare(t *testing.T, podUID, containerID1, containerID2 string) {
	...
	for _, c := range containers {
		for _, ss := range subSystems {
			podDir := path.Join("/fake", ss, "kubepods", "burstable", "pod"+podUID)
