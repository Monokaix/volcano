...
func Test_historicalUsageCalculator_utilizationMonitoring(t *testing.T) {
	...
	mgr, err := metriccollect.NewMetricCollectorManager(nil, nil)
	assert.NoError(t, err)
	...
	tests := []struct {
		...
		{
			name:        "calculate using native cpu&memory",
			...
			getPodsFunc: func() ([]*v1.Pod, error) {
				return []*v1.Pod{
					...
					utiltesting.MakePod("offline-1", 1, 1000, ""),
				}, nil
			},
			prepare: func() {
		...
		//{
		//	name:        "calculate using native cpu&memory exclude guaranteed pods",
		//	usages:      workqueue.NewNamedRateLimitingQueue(nil, "calculator"),
		//	getNodeFunc: makeNode,
		//	getPodsFunc: func() ([]*v1.Pod, error) {
		//		return []*v1.Pod{
		//			utiltesting.MakePod("online-1", 3, 2000, ""),
		//			utiltesting.MakePod("online-2", 2, 2000, ""),
		//			utiltesting.MakePod("offline-1", 1, 1000, "BestEffort"),
		//		}, nil
		//	},
		//	prepare: func() {
		//		err = os.Setenv("KUBELET_ROOT_DIR", dir)
		...
		//		assert.NoError(t, err)
		//	},
		//	expectRes: []apis.Resource{{v1.ResourceCPU: 0, v1.ResourceMemory: 1800}},
		//},
		//{
		//	name:        "total cpu request > node.Allocatable, cpu manager policy static, compute except guaranteed pods' cpu",
		//	usages:      workqueue.NewNamedRateLimitingQueue(nil, "calculator"),
		//	getNodeFunc: makeNode,
		//	getPodsFunc: func() ([]*v1.Pod, error) {
		//		limits := v1.ResourceList{
		//			v1.ResourceCPU:    *resource.NewQuantity(5, resource.DecimalSI),
		//			v1.ResourceMemory: *resource.NewQuantity(5000, resource.BinarySI),
		//		}
		//		burstablePod := utiltesting.MakePod("online-1", 4, 2000, "")
		//		burstablePod.Spec.Containers[0].Resources.Limits = limits
		//		return []*v1.Pod{
		//			burstablePod,
		//			utiltesting.MakePod("online-2", 5, 2000, ""),
		//			utiltesting.MakePod("offline-1", 2, 1000, "BestEffort"),
		//		}, nil
		//	},
		//	prepare: func() {
		//		err = os.Setenv("KUBELET_ROOT_DIR", dir)
		...
		//		assert.NoError(t, err)
		//	},
		//	expectRes: []apis.Resource{{v1.ResourceCPU: 1800, v1.ResourceMemory: 1800}},
		//},
		//{
		//	name:        "calculate using extend cpu&memory",
		//	usages:      workqueue.NewNamedRateLimitingQueue(nil, "calculator"),
		//	getNodeFunc: makeNode,
		//	prepare: func() {
		//		err = os.Setenv("KUBELET_ROOT_DIR", dir)
		...
		//			utiltesting.MakePod("online-1", 3, 2000, ""),
		//			utiltesting.MakePod("online-2", 2, 2000, ""),
		//			utiltesting.MakePodWithExtendResources("offline-1", 1000, 1000, "BestEffort"),
		//		}, nil
		//	},
		//	expectRes: []apis.Resource{{v1.ResourceCPU: 2400, v1.ResourceMemory: 1200}},
		//},
	}
	...
	}{
		{
			...
			getPodsFunc: func() ([]*v1.Pod, error) {
				return []*v1.Pod{
					utiltesting.MakePod("online-1", 3, 2000, ""),
					utiltesting.MakePod("online-2", 2, 2000, ""),
					utiltesting.MakePod("offline-1", 2, 2000, "BestEffort"),
				}, nil
			},
			expectedResource: framework.NodeResourceEvent{
				MillCPU:     1000,
